                    ...
                    if(dataSnapshot.child((i).toString()).child("category").value.toString().equals(cateName)){
                        ...
                        database.addListenerForSingleValueEvent(object : ValueEventListener {
                            override fun onDataChange(ds: DataSnapshot) {
                                comName = "Company: " + ds.child(companyId).child("user_name").value.toString()
                                position = ds.child(companyId).child("user_address").value.toString()
                                jobList.add(Job("Position: " + dataSnapshot.child((i).toString()).child("position").value.toString(), comName, position, dataSnapshot.child((i).toString()).child("requirement").value.toString(), dataSnapshot.child((i).toString()).child("id").value.toString()))
